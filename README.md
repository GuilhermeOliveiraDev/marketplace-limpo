...
- **Match exato** — esconde anúncios que não contêm todas as palavras da sua busca (a maior fonte de lixo: o FB adora mostrar coisa vagamente relacionada). Conectivos ("de", "com", "para"...) são ignorados. A regra é **letra frouxa, número exato**: `fold` casa dentro de "zfold" e de "cadeira" dentro de "cadeiras", mas `7` só casa o número 7 inteiro (não "70", não "512") e só grudado na palavra que veio junto na busca. Buscar `fold 7` acha "z fold 7", "zfold 7", "zfold7", "fold7" — e derruba "Z Fold 6", "Fold 8" e "Fold 6 com 7 meses de uso". O preço do card fica de fora da comparação.
- **Palavras proibidas** — blacklist livre, uma por linha ("aceito troca", "leia a descrição", "defeito", "fold 6"...). Casa **palavra inteira**: `ar` derruba "câmara de ar" e não encosta em "escape dizars" nem em "câmara". Por isso plural não vem de graça — `defeito` não pega "defeitos"; ponha as duas formas se quiser as duas. O espaço entre as palavras do termo é opcional: `z fold 6` pega "z fold 6", "zfold 6", "zfold6" e "z fold6". O contrário não vale — `zfold 4` digitado grudado só pega grudado, então escreva com espaço.
